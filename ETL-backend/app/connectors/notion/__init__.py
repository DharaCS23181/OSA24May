...
def notion_databases(
    ...
    if database_ids is None:
        ...
        database_ids = [
            {
                "id": result["id"], 
                "use_name": result["title"][0]["plain_text"] if result.get("title") and len(result["title"]) > 0 else "Untitled Database"
            }
    ...
    for database in database_ids:
        if "use_name" not in database:
            ...
            # Extract the name/title from the details safely
            database["use_name"] = details["title"][0]["plain_text"] if details.get("title") and len(details["title"]) > 0 else "Untitled Database"
