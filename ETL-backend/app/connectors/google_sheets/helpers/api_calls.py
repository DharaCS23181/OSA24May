...
def api_auth(credentials: GcpCredentials, max_api_retries: int) -> Resource:
    ...
    # 1. Try to detect if this is a service account (either via class or dict content)
    is_service_acc = isinstance(credentials, GcpServiceAccountCredentials)
    
    if is_service_acc:
        # DLT might have stuffed the entire JSON into project_id if it was passed as a single blob
        if isinstance(getattr(credentials, "project_id", None), dict):
            ...
            info = {
                ...
                "private_key_id": getattr(credentials, "private_key_id", None),
                "private_key": credentials.private_key,
                "client_email": credentials.client_email,
                "client_id": getattr(credentials, "client_id", None),
                "auth_uri": getattr(credentials, "auth_uri", "https://accounts.google.com/o/oauth2/auth"),
                "token_uri": getattr(credentials, "token_uri", "https://oauth2.googleapis.com/token"),
            }
        
        try:
            native_creds = service_account.Credentials.from_service_account_info(info).with_scopes(scopes)
        except Exception:
            # Fallback to DLT's native method if reconstruction fails (e.g. for default credentials)
            native_creds = credentials.to_native_credentials()
            if hasattr(native_creds, "with_scopes"):
                native_creds = native_creds.with_scopes(scopes)
    else:
        # Handle OAuth or other credential types
